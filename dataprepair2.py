...
def remove_empty_review():
    ...
    diff = userdf.merge(emptydf, indicator=True,
...
def addVenue(df, venueName, venue_location):
    key = 1
    ...
    if len(df) > 0:
        ...
        if venueName not in df.name.values:
            ...
            key = len(df) + 1
            newVenue = pd.DataFrame([[key, venueName, venue_location]], columns=[
    ...
    for username in onlyfiles:
        ...
        with alive_bar(len(review_from_user), title=titlebar) as bar:
            for index, row in review_from_user.iterrows():
                ...
                venueid, venuedf = addVenue(
                    venuedf, venue_name, venue_location)
                newRating = pd.DataFrame([[venueid, userid, score, time, comment]], columns=[
                                         'venueid', 'userid', 'score', 'time', 'comment'])
