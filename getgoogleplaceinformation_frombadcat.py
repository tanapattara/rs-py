...
# load venue
uncategory = pd.read_csv("data/badcategorydf.csv", sep='|', encoding='utf-8')

path_to_file = 'data/categorydf.csv'
if os.path.exists(path_to_file):
    categorydf = pd.read_csv("data/categorydf.csv", sep='|', encoding='utf-8')

path_to_file = 'data/badcategorydf2.csv'
badcategorydf = pd.DataFrame()
if os.path.exists(path_to_file):
    badcategorydf = pd.read_csv("data/badcategorydf2.csv", sep='|', encoding='utf-8')


with alive_bar(len(uncategory)) as bar:  
    for i, row in uncategory.iterrows():
        bar()
        venue_name = row['name']
        ...
        try:
            ...
            try:
                listofresult = driver.find_element(By.CSS_SELECTOR, '.Nv2PK.tH5CWc.THOPZb')
                strcate = listofresult.text.split('\n')
                strCategory = strcate[2].split(' ')[0]
            except:
                ...
                badcategorydf.to_csv('data/badcategorydf2.csv', sep='|', encoding='utf-8',index=False)
        driver.close()
        if strCategory == "":
            continue
