...
class RSKNN:
    ...
    def recommend2user(self, targetid, targetvenue):
        ...
        target_ratingdf = target_ratingdf.sort_values(by = 'score', ascending=False)
        target_top_score = target_ratingdf.iat[0,0]
        # find similarity of vanue
        find = []
        for i in range(2, 20):
            find.append(self.find_similar_venue(target_top_score, targetvenue, i))
    ...
    def find_similar_venue(self, venue_id, targetvenue, k):
        ...
        k += 1
        kNN = NearestNeighbors(n_neighbors=k, algorithm="brute", metric=self.metric)
        kNN.fit(self.X)
